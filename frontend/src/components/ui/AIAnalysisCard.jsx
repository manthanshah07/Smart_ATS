...
import { Sparkles, CheckCircle2, AlertCircle, TrendingUp, Cpu, HelpCircle, Layers } from 'lucide-react'
...
export const AIAnalysisCard = ({
    ...
    return (
      <Card className={cn('border-dashed border-border bg-muted/20', className)}>
        <CardContent className="py-8 text-center text-muted-foreground">
          ...
          <p className="font-medium text-sm">AI Match Evaluation Pending</p>
          <p className="text-xs text-muted-foreground mt-1">
            Analysis is computed upon submission using Sentence Transformers and NLP extraction.
          </p>
        </CardContent>
      </Card>
    )
  ...
  return (
    <Card className={cn('border-border/80 shadow-sm bg-card overflow-hidden', className)}>
      {/* Header with Model Provenance Badge */}
      <div className="border-b bg-muted/30 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="font-semibold text-sm text-foreground">Explainable AI Match Breakdown</h4>
            <p className="text-xs text-muted-foreground">
              {role === 'recruiter' ? 'Deterministic candidate-to-job fit analysis' : 'Transparent evaluation for your submitted resume'}
            </p>
          </div>
        </div>
      ...
      <CardContent className="p-6 space-y-6">
          ...
          <div className="sm:col-span-4 text-center sm:text-left border-b sm:border-b-0 sm:border-r border-border/60 pb-3 sm:pb-0 sm:pr-4">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              {overall_match_score >= 80
                ? 'Strong match for requirements'
                : overall_match_score >= 60
                ? 'Moderate alignment with role'
          ...
          <div className="sm:col-span-8 space-y-2.5">
            ...
            <div>
              <div className="flex justify-between text-xs mb-1 font-medium">
                ...
                <span className="text-muted-foreground">{Math.round(semantic_similarity_score)}%</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs mb-1 font-medium">
                ...
                <span className="text-muted-foreground">{Math.round(skill_match_score)}%</span>
              </div>
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
            ...
            <div>
              <div className="flex justify-between text-xs mb-1 font-medium">
                ...
                <span className="text-muted-foreground">{Math.round(experience_match_score)}%</span>
              </div>
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4 space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400">
              ...
              <span>Matched Required Skills ({matched_skills.length})</span>
            </div>
        ...
        <div className="rounded-lg border bg-muted/30 p-4 space-y-2">
          <h5 className="text-xs font-semibold text-foreground uppercase tracking-wider flex items-center gap-1">
            <HelpCircle className="h-3.5 w-3.5 text-primary" /> AI Evaluation Summary
          </h5>
